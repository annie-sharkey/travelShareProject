...
export class TypeSelector extends Component {
  ...
  handleInputAdding = categoryIndex => {
    ...
    this.writeInputTextList(
      this.state.inputText_list.concat([itineraryItem]),
      this.state.list_of_categories
    );
  };
  ...
  handleInputEditing = categoryIndex => {
    ...
    var currentCardTextObjects = this.state.inputText_list.filter(
      result => result.categoryIndex === categoryIndex
    );

    currentCardTextObjects[
      currentCardTextObjects.length - 1
    ] = editedItineraryItem;

    var latestEditedItineraryItem = currentCardTextObjects.splice(
      currentCardTextObjects.length - 1,
      1
    );

    var noncurrentCardTextObjects = this.state.inputText_list.filter(
      result => result.categoryIndex !== categoryIndex
    );

    var finalEditedInputsForAllCards = noncurrentCardTextObjects.concat(
      latestEditedItineraryItem
    );
  ...
  onAddingCard = () => {
    this.setState({
      ...this.state,
      list_of_categories: this.state.list_of_categories.concat([
        this.state.category
      ]),
      category: ""
    });

    this.writeListOfCategories(
      this.state.inputText_list,
      this.state.list_of_categories.concat([this.state.category])
    );
  };

  onEditingCard = categoryIndex => {
    var allCurrentCardTextToEdit = this.state.inputText_list
      .filter(result => result.categoryIndex === categoryIndex)
      .map(result => result.text_to_display);
      ...
      <div>
        <div className="selector">
          ...
          <FloatingActionButton
            onTouchTap={() => this.onAddingCard(this.state.category)}
            mini={true}
            style={style}
          >
            <ContentAdd />
          ...
          <div>
            ...
            <RaisedButton
              primary={true}
              label={this.submitState ? "Submit" : "ReSubmit"}
              onTouchTap={
                this.submitState
                  ? () => this.handleInputAdding(this.index_tracker)
                  : () => this.handleInputEditing(this.index_tracker)
              }
              style={style}
            />
          </div>
            ...
            return (
              ...
              >
                ...
                <CardText>
                  {this.state.inputText_list
                    .filter(result => result.categoryIndex === categoryIndex)
                    .map(result => <div>{result.text_to_display}</div>)}
                </CardText>
                <CardActions>
                  <RaisedButton
                    label="Edit"
                    primary={true}
                    onTouchTap={() => this.onEditingCard(categoryIndex)}
                    style={style}
                  />
